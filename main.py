...
def load_data():  
    data = np.empty((164,3,256,256),dtype="float32")  
    label = np.empty((164,1,),dtype="uint8")  
    
    labels = os.listdir("./sample") 
    # print(labels)
    # print(label)
    # print(len(label))
    numl = len(labels)
    c=0
    for n in range(numl):
        imgs = os.listdir("./sample/"+labels[n])  
        num = len(imgs)
        for i in range(num):
            if '.png' in imgs[i]:
                img = Image.open("./sample/"+labels[n]+"/"+imgs[i])
                # print(len(img)) 
                arr = np.asarray(img,dtype="float32")
                # print(arr)
                data[c,:,:,:] = [arr[:,:,0]/256,arr[:,:,1]/256,arr[:,:,2]/256]
                
                label[c][0] = int(labels[n])
                c+=1
                # print(labels[n])
                # print(label[i][0])
    return data,label 
...
model.add(Convolution2D(16, (5, 5), border_mode='valid', input_shape=(3,256,256)))#input_shape在后面的层可以推导出来，不需要为每一个层都指定这个参数  
...
model.add(Flatten())  
model.add(Dense(300))#256=16*4*4  input_dim=256, output_dim=128
# model.add(Activation('relu')) 
model.add(Dropout(0.5))
model.add(Activation('tanh'))
# model.add(Dense(300))#256=16*4*4  
# model.add(Dense(300))
# model.add(Activation('tanh'))
...
model.fit(data, label, batch_size=20, nb_epoch=100,shuffle=True,verbose=1)#,show_accuracy=True,validation_split=0.2\

a=model.predict(data[0:100])
